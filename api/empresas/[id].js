...
module.exports = async (req, res) => {
    ...
    if (!id) {
        sendJson(res, 400, { error: 'ID da empresa não informado.' });
        return;
    }
    ...
    if (req.method === 'PUT') {
        try {
            ...
            const index = data.empresas.findIndex(e => String(e.id) === String(id));
    ...
    if (req.method === 'DELETE') {
        const data = readData();
        const filtered = data.empresas.filter(e => String(e.id) !== String(id));

        if (filtered.length === data.empresas.length) {
            sendJson(res, 404, { error: 'Empresa não encontrada.' });
            return;
        }
